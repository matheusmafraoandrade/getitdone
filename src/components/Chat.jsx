...
export default function Chat({
  ...
  const sendMessage = async (e) => {
    e.preventDefault();
    if (messageContent === "") return; // Evita enviar strings vazias ao clicar no botão
    let chatId;
  ...
  return (
    <div className="fixed bottom-16 right-8 sm:right-[212px] h-[450px] w-[336px] sm:w-[380px] border border-foreground/10 bg-input rounded-lg p-5 drop-shadow-2xl">
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-3">
          ...
          <p className={`${selectedChat ? "" : "text-lg"} font-medium`}>
        ...
        <form onSubmit={sendMessage} className="flex gap-2.5 mt-1">
          <Input
            type="text"
